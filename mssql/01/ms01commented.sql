...
SET @FullRowResult = 1              /* Output full matching rows, 0 or 1*/
...
SET @SearchStrColumnName = NULL     /* Limit search to a specific column name, NULL for all columns, uses LIKE syntax */
SET @SearchStrInXML = 1             /* Include XML columns, 0 or 1. Searching XML data may be slow */
...
BEGIN
    ...
    BEGIN
        ...
        SET @sql = 'SELECT QUOTENAME(COLUMN_NAME),DATA_TYPE
            ...
            AND DATA_TYPE IN (' + --data type filter
                CASE 
        ...
        BEGIN
            ...
            SET @sql = 'SELECT ''' + @TableName + ''',''' + @ColumnName + ''',' + 
                CASE @ColumnType 
                    WHEN 'xml' THEN 'LEFT(CAST(' + @ColumnName + ' AS nvarchar(MAX)), 4096),''' --handle XML columns
                    WHEN 'timestamp' THEN 'master.dbo.fn_varbintohexstr('+ @ColumnName + '),''' 
                    ELSE 'LEFT(' + @ColumnName + ', 4096),''' 
                END + @ColumnType + '''
                FROM ' + @TableName + ' (NOLOCK) ' + ' WHERE ' + 
                CASE @ColumnType 
                    WHEN 'xml' THEN 'CAST(' + @ColumnName + ' AS nvarchar(MAX))' --search XML columns
                    WHEN 'timestamp' THEN 'master.dbo.fn_varbintohexstr('+ @ColumnName + ')'
                    ELSE @ColumnName 
