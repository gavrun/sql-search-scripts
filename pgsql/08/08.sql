...
DECLARE
    ...
    -- Filters 
    schema_name_filter TEXT := 'public'; -- LIKE pattern for schema names (e.g., 'public', 'app%', NULL for all non-system)
    table_name_filter TEXT := NULL; -- LIKE pattern for table names (NULL for all)
    column_name_filter TEXT := NULL; -- LIKE pattern for column names (NULL for all)
    max_string_preview_length INT := 128;
...
BEGIN
    ...
    WHERE t.table_type = 'BASE TABLE'
      ...
      AND (schema_name_filter IS NULL OR t.table_schema LIKE schema_name_filter) -- Apply filters
      AND (table_name_filter IS NULL OR t.table_name LIKE table_name_filter);
    ...
    FOR table_rec IN SELECT table_id, schema_name, table_name, quoted_table_name FROM temp_table_metadata LOOP
        ...
        WHERE c.table_schema = table_rec.schema_name
          ...
          AND (column_name_filter IS NULL OR c.column_name LIKE column_name_filter); -- Apply column filter
